		...
		<div class="modal-body" style="width: 100%;">
			...
			<div>
				<table class="table table-striped table_pag" style="margin-top: 1rem; margin-bottom: 0 !important;">
					...
					<tbody>
						<?php
							if (!$loParcelas->hasPagamento()) { ?>
								<tr>
									<td colspan="7">Não há pagamentos lançados para o empréstimo.</td>
								</tr>
						<?php } else {
								foreach ($loParcelas as $oParcela) {
									$loPagamentoList = Sistema::PagamentoDAO()->findByParcela($oParcela);
	
									if ($loPagamentoList->isEmpty()) {
										continue;
									}

									foreach ($loPagamentoList as $oPagamento) {
										$fTotalPago += $oPagamento->getValor();
						?>
									<tr>
										<td><?php echo $oEmprestimo->getId(); ?></td>
										<td><?php echo $oPagamento->getId(); ?></td>
										<td><?php echo "{$oParcela->getSequenciaParcela()}ª"; ?></td>
